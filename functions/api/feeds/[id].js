...
export async function onRequestPatch(context) {
    try {
        ...
        const { order } = body;
        if (order === undefined) {
            return new Response(JSON.stringify({ error: 'Order field is required' }), { status: 400, headers: { 'Content-Type': 'application/json' } });
        }
        // Update feed order
        const updated = await updateDocument(feedPath, { order }, idToken, config);
        if (!updated) {
            return new Response(JSON.stringify({ error: 'Failed to update feed' }), { status: 500, headers: { 'Content-Type': 'application/json' } });
        }
        return new Response(JSON.stringify({ success: true }), {
            status: 200,
            headers: { 'Content-Type': 'application/json' },
