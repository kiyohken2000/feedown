...
const MAX_RETRIES = 3;
async function updateUserStateWithRetry(uid, articleIds, idToken, config) {
    const debugInfo = [];
    for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
        try {
            ...
            const currentIds = userState?.readArticleIds || [];
            const currentSet = new Set(currentIds);
            debugInfo.push(`currentIds:${currentIds.length}`);
            // Filter out already read articles
            const newIds = articleIds.filter((id) => !currentSet.has(id));
            ...
            const success = await setDocument(`users/${uid}/userState/main`, {
...
export async function onRequestPost(context) {
    try {
        ...
        if (!articleIds || !Array.isArray(articleIds) || articleIds.length === 0) {
            return new Response(JSON.stringify({ error: 'articleIds array is required' }), { status: 400, headers: { 'Content-Type': 'application/json' } });
        }
        ...
        return new Response(JSON.stringify({ success: true, added: 0, total: 0 }), { status: 200, headers: { 'Content-Type': 'application/json' } });
    }
}
