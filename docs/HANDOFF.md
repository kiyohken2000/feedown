...
```bash
# Web版デプロイ（ルートディレクトリから、パス引数なしで実行すること！）
cd /path/to/feedown
npm run build --workspace=apps/web
npx wrangler pages deploy
```

**重要**: `wrangler pages deploy` はルートディレクトリから、**パス引数を付けずに**実行すること。
`wrangler.toml` の `pages_build_output_dir` が使われる。`apps/web/dist` などのパス引数を付けると
functionsフォルダが含まれず、APIが405エラーになる。
...
# 4. Web版をデプロイ（キャッシュクリアのため。ルートからパス引数なしで実行）
npm run build --workspace=apps/web
npx wrangler pages deploy
```
