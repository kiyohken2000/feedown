# オンデバイスRSSリーダーに「翻訳特化LLM」を後付けして、要約は executorch、翻訳は llama.rn の二刀流にした話
...
1 は executorch のモデル選択肢を上げるだけで実装は楽だが、要約・チャットも一緒に遅く・重くなる副作用がある。Phase F で実測したところ、LFM 2.5 1.2B は同サイズの汎用モデル比でかなり高速（後述）、これを差し替えるのは惜しかった。
...
最終的に 2 を選んだ。理由は次の Phase E / Phase F の実測結果。
...
## Phase F: 汎用LLMも llama.rn にすべき? → No
