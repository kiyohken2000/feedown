...
### Gemma 4 E2B 検証 (executorch 0.9.1, 2026-06-13 検証済み / 条件付き採用)

#### 結論

**1.0.13 で `recommendation: 'candidate'` + `minDeviceRamBytes: RAM_TIER.TIER_8GB`
ゲート付きで採用** (`apps/mobile/src/ai/models.js`)。
4GB 級端末では `canRunOnDevice()` が選択肢自体を隠す。デフォルトは引き続き
...
| 端末 | RAM | 結果 | 備考 |
|---|---|---|---|
| iPhone 13 mini (A15) | 4 GB | **jetsam OOM 確定** | DL 後の最初の generate で落ちる。`poc/llama-rn-gemma4` で観察した GGUF と同じ「~2.5GB + RN baseline」の物理壁。 |
| iPhone Air (A19 Pro) | 11.5 GB | **動作 OK** | 初回 cold load ~54s (Metal shader compile)、以降キャッシュ。Warm decode ~121 ch/s。 |

**品質**: 5000 char 程度の英記事要約で `LFM2.5-1.2B Instruct` (~209 ch/s) と
比較したが、品質差は小さく速度は約 1.7 倍遅い。「同等品質をより重いメモリで」
の典型で、デフォルト昇格の理由は薄い。

#### 採用判断

1. **デフォルトにはしない**。LFM 2.5 1.2B より遅く品質差も小さいため、
   既定モデルを変える理由が無い。
2. **`recommendation: 'candidate'` で選択肢として残す**。iPhone Air / iPhone 16 Pro
   など 8GB+ 端末を使うユーザーが Gemma 系を試したいケースに備える。
3. **RAM ゲート必須**。4GB 端末では `canRunOnDevice()` で UI から非表示にし、
   AsyncStorage に Gemma 4 が選択済みで保存されていても起動時に
   `selectedModel` を `FEEDOWN_DEFAULT_LLM_ID` にフォールバックする
   (`apps/mobile/src/contexts/AiContext.js`)。
4. **8GB は保守的な閾値**。実検証済みは 4GB (NG) と 11.5GB (OK) のみで、
   6GB 端末 (iPhone 15 Pro 等) は未検証。`RAM_TIER.TIER_8GB` (~7.0 GB)
   を要求することで実害が出にくい線を引いた。検証端末が増えたら緩める。

#### MLX backend のバイナリ影響

iOS バイナリに `mlx.metallib` + `libbackend_mlx_ios.a` が追加されるが、サイズ
増分は許容範囲 (1.0.13 のリリースビルドで App Store 警告なし)。将来 Gemma 4 を
外す決定をしても 0.9.1 自体は据え置き可能。
