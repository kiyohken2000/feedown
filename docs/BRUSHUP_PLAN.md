...
進捗と「見送り（⏸）」の判断をここに残す。⏸ は**恒久却下ではなく、そのセッションでの優先度判断による保留**。再開時はこの記録を読んでから着手/再判断すること。

### 2026-07-05 セッション

- **完了・出荷**: #1（期限切れ記事クリーンアップ、本番デプロイ済）、#5（Web リーダーモード、同）、#10（Web キーボードショートカット、同）、#13（README/docs 同期、docsのみ）。
- **見送り（⏸）** — いずれも「モバイル変更なしで完結できる web/functions/docs 系」の中からユーザーが今回の作業対象を絞った結果。技術的な却下理由ではなく着手範囲の優先度判断:
  - **#2 バックグラウンド自動更新**: P1 だが規模「中」で新規 Worker + Cron + `CRON_SECRET` 認証が必要。今回は小〜中の即効性ある項目を優先し保留。次に信頼性を固めるなら最有力。
  - **#3 CI (GitHub Actions)**: 保留。着手は容易だが、現状 `eslint .` に既存エラー（未使用変数など）が複数あり、CI を緑にするには先にそれらの lint 修正が要る点に留意。
...
プロダクト方針（15年来の RSS リーダーユーザー向け・シンプルさ重視・無料枠運用）に沿う範囲の新機能。各項目は独立しており、価値が薄いと判断したら個別に見送ってよい。**却下した場合はこの表に「見送り + 理由」を書き残すこと**（過去に OPML モバイル版・プッシュ通知・Web 共有ボタンを「不要」と判断した経緯があり、同じ議論を繰り返さないため）。

### 18. フィード自動発見（サイト URL からの追加）【P2・小】

**問題**: `POST /api/feeds` は直近の修正（`5c6119a`）で HTML ページを**拒否**するようになった。正しい防御だが、ユーザー体験としては「サイトの URL を貼ったらフィードを見つけてほしい」が本来の理想形。RSS URL を探し当てられる人しかフィードを追加できないのは、初心者への障壁。

**実装方針**:
- `functions/api/feeds/index.ts` の POST で、取得結果が HTML だった場合に即拒否せず、`<link rel="alternate" type="application/rss+xml">` / `application/atom+xml` を探す（linkedom は `article-content.ts` で導入済み）。
- 見つかった候補 URL を再取得してフィードとして検証し、通れば**その URL で**登録。レスポンスに `discoveredFrom` を含め、UI で「サイト URL からフィードを検出しました: {url}」と表示。
- 候補が複数ある場合（RSS と Atom 併設等）は最初の 1 件でよい。見つからなければ従来どおりのエラー。
- リダイレクトは 3 回まで、fetch タイムアウトは既存実装に合わせる。
- Web `FeedsPage.jsx` / Mobile `Read.js` のエラーメッセージ文言を更新。
...
**実装方針**:
- スキーマ: `feeds` に `folder TEXT DEFAULT NULL` を追加（専用テーブルは過剰。フォルダ名の実体はフィード行に持たせる）。マイグレーション SQL を `docs/SUPABASE_SETUP.md` に追記し、**既存セルフホスター向けの ALTER TABLE 手順を明記**。
- API: `GET/POST/PATCH /api/feeds` で `folder` を受け渡し。
- Web: `FeedsPage.jsx` でフォルダごとのグループ表示（折りたたみ可）、フィード編集でフォルダ名の設定・変更。`DashboardPage.jsx` のフィード選択ドロップダウンにフォルダ見出し + 「フォルダ全体」選択肢（`feedId` の複数指定 or `folder` パラメータを API に追加）。
- Mobile: フィードドロップダウンにフォルダ見出し表示。編集 UI は Web 優先で、Mobile は表示のみから始めてよい。
- OPML: エクスポートで `<outline>` をネスト、インポートで親 outline 名を `folder` に反映（`FeedsPage.jsx` 内の既存実装を拡張）。
...
**実装方針**:
- 保存先: `user_profiles` に `mute_keywords TEXT[] DEFAULT '{}'` を追加（専用テーブルは不要。上限 50 件程度を API 側で強制）。
- 適用場所: **クエリ時**にフィルタ（`functions/api/articles/index.ts` で `NOT ilike ANY`）。保存時に落とす方式は「ルールを消しても記事が戻らない」ので不採用。未読件数 (#7) のカウントにも同じフィルタを適用して整合させる。
- UI: Web `SettingsPage.jsx` / Mobile `Profile.js` にキーワード追加・削除 UI。
- `docs/API.md` / `docs/SUPABASE_SETUP.md` 更新。
...
**実装方針**:
- Home のヘッダーかタブに「Digest」導線を追加。未読記事のうち**要約キャッシュが存在するもの**を新しい順に最大 20 件、要約カード（フィード名 + タイトル + brief 要約 3 行）で表示。タップで ArticleDetail へ。
- 要約が無い記事はその場で生成**しない**（一覧表示を重くしない）。「n 件は未要約」とだけ表示。
- 既存 TTS (`ttsService.js`) で上から連続読み上げボタンを付けると、AI 計画の将来拡張「音声ダイジェスト」の最小版になる（画面表示中のみ。バックグラウンド再生はしない）。
- 新規ネイティブ依存なし。
...
**実装方針**:
- まずは**クライアント側のみ・厳密一致**で始める: 正規化 URL（クエリの `utm_*` 等を除去）または正規化タイトル（空白・記号を圧縮）が一致する記事を 1 カードにまとめ、「他 n フィード」バッジで展開可能に。
- Web は `DashboardPage.jsx`（#11 の分割後なら `ArticleList`）、Mobile は `Home.js` の表示層のみ。API・スキーマ変更なし。
- 類似度ベース（embedding）のグルーピングは #17（意味検索）の基盤ができたら再検討。この項目ではやらない。
...
**実装方針**:
- `vite-plugin-pwa` を `apps/web` に追加。manifest（アイコンは既存ロゴ流用）+ service worker で**静的アセットのみ** precache。
- **API レスポンスは service worker でキャッシュしない**。`/api/*` は NetworkOnly に明示設定する（既存の `Cache-Control: no-store` 問題を SW レイヤーで再発させないこと。過去に Recommended Feeds のキャッシュ不整合でハマった経緯がある）。
- 記事データのオフライン表示までは踏み込まない（欲しくなったら別項目として起票）。
