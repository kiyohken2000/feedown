...
**最終更新**: 2026-07-05
**現在のフェーズ**: Phase 13 まで完了。以降はこのファイルでの追跡を凍結（下記参照）
**ステータス**: App Store / Google Play 公開済み（モバイル 1.0.13）

> **この進行表は Phase 13 で凍結しています。** 以降の実装（オンデバイス AI 機能、モバイル 1.0.9〜1.0.13 のリリース群など）は、このファイルには反映していません。最新の進捗は次を一次資料として参照してください。
> - オンデバイス AI（要約・翻訳・TTS・チャット・信号分離）: `docs/MOBILE_ON_DEVICE_AI_PLAN.md`（Phase 1〜6 出荷済み）
> - 進行中のブラッシュアップ計画: `docs/BRUSHUP_PLAN.md`
> - その他の変更履歴: `git log`
...
### モバイルアプリ トークンリフレッシュ機能
- [x] `/api/auth/refresh` エンドポイント新規作成
- [x] ログイン/登録APIで `refreshToken` を返すように修正
- [x] モバイルアプリで `refreshToken` を保存・使用
- [x] 401エラー時に自動でトークンをリフレッシュしてリトライ

### Recommended Feeds キャッシュ問題修正
- [x] `Cache-Control: no-store` に変更してブラウザキャッシュを無効化
