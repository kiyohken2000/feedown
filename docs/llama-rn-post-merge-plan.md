...
## 追加検証: 翻訳特化モデル (フェーズ E, 2026-05-30)

「全面移行 NO-GO」後の残課題「特定 GGUF only モデルが要るか?」を、**翻訳機能**でだけ検証した。現行の翻訳パス (`useArticleTranslation` + LFM2.5 1.2B + `buildTranslationMessages`) は本文を 2500 字に切り詰めて 1 回の LLM 呼び出しで処理しており、長文記事で品質が落ちる。

### 検証対象: Tencent **Hy-MT2 1.8B Q4_K_M** (1.13GB, 33 言語の "fast-thinking" 翻訳特化 GGUF)

- 公式テンプレ (system 無し / 単一 user turn / `**only output the translated result**` ガード)
- 推奨サンプリング (temp 0.7, top_p 0.6, top_k 20, rep_penalty 1.05)
- 同一 EN 記事 (`BENCH_TRANSLATE_SOURCE`, iOS 27 発表記事, ~1.1k 字) で LFM2.5 1.2B と並べた

### 結果 (iPhone 13 mini, A15/4GB)

| モデル | サイズ | loadMs | tok/s | 翻訳品質 |
|--------|--------|--------|-------|----------|
| Hy-MT2 1.8B Q4_K_M (llama.rn) | 1.13GB | ~2300 | 20.7 | **段落構造を保持、情報欠落なし、自然な日本語** |
| LFM2.5 1.2B (executorch, 現行) | ~700MB | - | - | **段落を圧縮、情報欠落あり、JSON ガードで歪む** |

- **GPU working-set**: Hy-MT2 Q4 は 1.13GB で A15 の 2863MB 内に十分収まる (Gemma 4 のような RAM 壁問題なし)
- **2-bit / 1.25-bit Hy-MT2 GGUF は使用不可**: AngelSlim STQ kernel が unmerged llama.cpp PR #19357 依存、現行 llama.rn では tensor offset エラー
- → Q4_K_M / Q6_K (標準 quant) が現実的な選択肢

### 追加検証: 汎用 LLM (フェーズ F, 2026-05-30)

「翻訳以外も llama.rn にする価値があるか?」を、同じ要約プロンプト (`BENCH_MESSAGES`, 2 文要約) で比較。

| モデル | サイズ | loadMs | tok/s | 出力 |
|--------|--------|--------|-------|------|
| Gemma 3 1B-IT Q4_K_M (llama.rn) | 806MB | 1935 | 19.1 | 2 文 ✓ / release timing 欠落 |
| LFM2.5 1.2B (executorch, 現行) | ~700MB | 2923 | **29.8** | 3 文 / release timing 含む |

- **同じ A15 GPU で executorch が 55% 速い** (29.8 vs 19.1 tok/s)。同サイズ汎用モデル比較で決定的
- 出力品質は LFM2.5 のほうがむしろ豊か (Gemma は format 厳守だが内容で負け)
- → 汎用 LLM を llama.rn にする根拠はゼロ

### 修正された最終判断: **部分採用 (翻訳のみ llama.rn + Hy-MT2)**

| 機能 | バックエンド | モデル |
|------|------------|--------|
| 要約 / signal / chat / TTS | **executorch (現行維持)** | LFM2.5 1.2B |
| **翻訳のみ** | **llama.rn (新規追加)** | Hy-MT2 1.8B Q4_K_M |

- "2 バックエンド保守コスト" の懸念は残るが、**翻訳機能の品質ギャップが具体的で、執筆者用途の RSS リーダーとして無視できない大きさ** だったため許容
- 同居しないで動的ロード/アンロード (翻訳開始時に LFM2.5 をアンロード → Hy-MT2 ロード → 翻訳完了後に逆) で 4GB 端末の RAM 制約をクリアする方針
- 端末 RAM ベースのゲーティング (3.5GB/5.3GB/7.0GB tier、`Device.totalMemory` 検出) を `src/ai/llamaRnPoC.js` に実装済み。本実装でも流用する

**次の作業**: load/unload swap の本実装設計 → `docs/MOBILE_ON_DEVICE_AI_PLAN.md` の翻訳セクション (Phase 4) を Hy-MT2 + llama.rn 前提で書き直す。
